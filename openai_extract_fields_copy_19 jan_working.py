...
# Suppress logs during server integration
SUPPRESS_LOGS = os.getenv("SUPPRESS_LOGS", "true").lower() == "true"

def log(message):
    """Conditional logging based on suppression flag."""
...
def analyze_with_openai(text):
    ...
    try:
        ...
        log(json.dumps(raw_response, indent=4))
        ...
        log(f"Raw OpenAI response saved to: {intermediate_output_path}")
        return valid_json
    except requests.exceptions.RequestException as e:
        log(f"Request error with OpenAI: {e}")
        return None
    except Exception as e:
        log(f"Error while analyzing with OpenAI: {e}")
        return None
...
def main():
    ...
    try:
        ...
        print(json.dumps(parsed_data))  # Only JSON is printed for server integration
    except Exception as e:
        print(json.dumps({"error": str(e)}))  # JSON error output
        sys.exit(1)
